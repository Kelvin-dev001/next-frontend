...
import {
  Box, Container, Grid, Typography, IconButton, Divider, Stack
} from '@mui/material';
...
export default function Footer() {
  return (
    <Box sx={{ background: bluishGradient, color: 'white', pt: 8, pb: 2, mt: 0, borderTop: '2.5px solid #1e3c72', boxShadow: '0 0 32px 0 rgba(30,60,114,0.13)' }}>
      <Container maxWidth="xl">
        <Grid container spacing={6} justifyContent="space-between">
          <Grid item xs={12} md={4} sx={{ minHeight: FOOTER_BRANDING_MIN_HEIGHT }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
              <img src="/snaap-logo.jpeg" alt="Snaap Connections" width={48} height={48} style={{ marginRight: 16, filter: 'drop-shadow(0 2px 8px #2a5298AA)' }} />
              <Typography variant="h6" fontWeight={700} sx={{ color: '#fff', textShadow: '0 2px 8px rgba(46,90,162,0.2)' }}>
                Snaap Connections
              </Typography>
            </Box>
              ...
              {socialLinks.map(({ icon, href, label }) => (
                <IconButton
                  ...
                  sx={{
                    ...
                    '&:hover': { bgcolor: 'rgba(255,255,255,0.35)', color: '#2a5298', transform: 'scale(1.17) rotate(-8deg)' }
                  }}
                  aria-label={label}
                >
                  {icon}
          ...
          {footerLinks.map(section => (
            <Grid item xs={12} sm={4} md={2.5} key={section.title} sx={{ minHeight: 80 }}>
              <Typography variant="subtitle1" fontWeight={700} sx={{ color: '#c4e0fc', letterSpacing: '1.1px', mb: 1.5 }}>
                {section.title}
              </Typography>
              <Stack spacing={1}>
                  ...
                  >
                    <Typography
                      sx={{
                        ...
                        '&:hover': { color: '#81c2ff', opacity: 1, textShadow: '0 2px 12px #c4e0fc55' }
                      }}
                    >
                      {link.label}
                    </Typography>
        ...
        <Divider sx={{ my: 4, borderColor: 'rgba(255,255,255,0.25)' }} />
        <Typography variant="body2" sx={{ color: '#e6f2ff', opacity: 0.7, textAlign: 'center' }}>
