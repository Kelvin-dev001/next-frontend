    ...
    <div id="products" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-4 py-10">
      {items.map((product) => (
        <Link href={`/products/${product._id}`} key={product._id} className="group block">
          <div className="relative bg-white shadow-lg hover:shadow-2xl rounded-xl overflow-hidden transition-all duration-300 transform group-hover:-translate-y-1 group-hover:scale-[1.02]">
            <div className="relative w-full h-56 overflow-hidden">
              {product.imageUrl ? (
                <>
                  {!loadedImages[product._id] && <div className="absolute inset-0 bg-gray-200 animate-pulse"></div>}
                  <img
                    src={getOptimizedCloudinaryUrl(product.imageUrl, { width: 400 })}
                    alt={product.name}
