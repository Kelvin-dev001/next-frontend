...
export default function DealsSection() {
  ...
  useEffect(() => {
    Api.getProducts("?isOnSale=true&limit=30")
      .then((res) => {
        const allDeals = res.products || [];
        setDeals({
        ...
        deals[key] && deals[key].length > 0 ? (
          <Box key={key} sx={{ mb: { xs: 6, md: 8 } }}>
            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2, px: 1 }}>
              <Typography variant="h5" sx={{ fontWeight: 700, color: "primary.dark", letterSpacing: 1, fontFamily: "'Montserrat', 'Roboto', sans-serif" }}>
                {label}
              </Typography>
            </Stack>
