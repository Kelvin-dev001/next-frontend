    ...
    <header className="sticky top-0 z-50 bg-white text-gray-900 shadow-[0_2px_24px_0_rgba(0,0,0,0.06)]">
      <div className="mx-auto flex min-h-16 max-w-screen-2xl items-center gap-2 px-3 md:min-h-[72px] md:px-6">
        ...
        <nav className="hidden items-center gap-2 md:flex">
          {navLinks.map((l) => (
            <Link key={l.href} href={l.href} className="px-2 py-1 text-sm font-medium hover:text-[#1e3c72]">{l.label}</Link>
          ))}
        ...
        <button onClick={openDrawer} className="text-[#1e3c72] md:hidden" aria-label="Search"><FaSearch className="text-xl" /></button>
      </div>

      {drawerOpen && (
        <>
          <div className="fixed inset-0 z-[1200] bg-black/40" onClick={closeDrawer} aria-hidden="true" />
          <div className="fixed right-0 top-0 z-[1300] h-full w-[92vw] max-w-[400px] overflow-y-auto rounded-l-3xl bg-gray-50 p-6 shadow-[0_8px_32px_#1e3c7222]">
            ...
            <ul>
              {menuSections.map((s) => (
                <li key={s.label}>
                  <Link href={s.link} onClick={closeDrawer} className={drawerItem}><span className="text-[#1e3c72]">{s.icon}</span> {s.label}</Link>
                </li>
              ))}
            </ul>
                ...
                <li key={cat._id || cat.name}>
                  {cat.name ? (
                    <Link href={`/products?category=${encodeURIComponent(cat.name)}`} onClick={closeDrawer} className={drawerItem}>
                      <FaThLarge className="text-[#1e3c72]" /> {cat.name}
                    </Link>
                  ) : (
                ...
                <li key={brand._id || brand.name}>
                  {brand.name ? (
                    <Link href={`/products?brand=${encodeURIComponent(brand.name)}`} onClick={closeDrawer} className={drawerItem}>
                      <SiBrandfolder className="text-[#1e3c72]" /> {brand.name}
                    </Link>
                  ) : (
