...
export default function MainNavbar({ onMenuClick }) {
          ...
          <Box sx={{ p: 3, pt: isMobile ? 4 : 3 }}>
            ...
            <List>
              {menuSections.map((section) => (
                <ListItem button key={section.label} onClick={() => handleMenuLinkClick(section.link)}>
            ...
            <List>
              {categories.map((cat) => (
                <ListItem button key={cat._id || cat.name} onClick={() => handleMenuLinkClick(getCategoryLink(cat))}>
                  <ListItemIcon>
                    <CategoryIcon />
                  </ListItemIcon>
            ...
            <List>
              {brands.map((brand) => (
                <ListItem button key={brand._id || brand.name} onClick={() => handleMenuLinkClick(getBrandLink(brand))}>
                  <ListItemIcon>
                    <SiBrandfolder />
                  </ListItemIcon>
