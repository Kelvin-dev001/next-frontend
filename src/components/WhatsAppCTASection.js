...
import { Box, Typography, Button, Stack, useTheme, useMediaQuery, Slide, Tooltip } from "@mui/material";
...
export default function WhatsAppCTASection() {
  ...
  return (
    <Box
      sx={{
        my: { xs: 5, md: 8 }, py: { xs: 4, md: 6 }, px: 2,
        borderRadius: "32px",
    ...
    >
      <style>{`
        ...
        .whatsapp-circular:hover, .whatsapp-circular:focus { transform: scale(1.09) rotate(-6deg); box-shadow: 0 8px 36px #25d36677; }
        .cta-btn-animated { transition: background 0.22s, color 0.22s, box-shadow 0.22s, transform 0.22s cubic-bezier(.4,2,.4,1); }
        .cta-btn-animated:active { transform: scale(0.97); }
        .cta-btn-animated:hover, .cta-btn-animated:focus { background: linear-gradient(96deg,#25d366 15%,#1e3c72 100%); color: #fff; box-shadow: 0 3px 24px #25d36655; }
        .bubble-anim { animation: bubbleIn 0.9s cubic-bezier(.4,2,.4,1); }
        @keyframes bubbleIn { 0% { opacity: 0; transform: translateY(25px) scale(0.96);} 60% { opacity: 1; transform: translateY(-6px) scale(1.07);} 100% { opacity: 1; transform: translateY(0) scale(1);} }
      `}</style>

      <Slide direction="up" in={showBubble} timeout={700}>
        <Box sx={{ position: "absolute", top: isMobile ? 10 : 18, left: isMobile ? 18 : 60, zIndex: 1, opacity: 0.14 }} className="bubble-anim">
      ...
      <Stack direction="column" alignItems="center" spacing={3} sx={{ zIndex: 2, position: "relative" }}>
        ...
        <Typography variant={isMobile ? "h5" : "h4"} fontWeight={800} sx={{ color: "#fff", textShadow: "0 2px 18px #1e3c72cc", letterSpacing: 1.2, fontFamily: "'Montserrat', 'Roboto', sans-serif", mb: 1 }}>
          Need Help? Chat with Us on WhatsApp!
        </Typography>
        <Typography variant="subtitle1" sx={{ color: "#e6f2ff", opacity: 0.92, mb: 2, mx: "auto", maxWidth: 480, fontWeight: 500, textShadow: "0 2px 10px #25d36644" }}>
        ...
        <Typography variant="body2" sx={{ mt: 1.8, color: "#d6f9e7", opacity: 0.8, textAlign: "center" }}>
          Available <b>8:00am – 8:00pm</b>, Monday to Saturday
        </Typography>
      </Stack>

      <Box sx={{ position: "absolute", left: 0, bottom: -4, width: "100%", zIndex: 1, pointerEvents: "none" }} aria-hidden="true">
